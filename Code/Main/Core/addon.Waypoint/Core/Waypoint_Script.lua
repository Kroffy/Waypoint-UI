...
function NS.Script:Load()
	...
	do
		do -- GET
			...
			function Callback:GetQuestContextIcon(questID)
			...
			function Callback:GetQuestInfo()
				...
				if isQuest and not isWorldQuest then
					...
					questInfo.contextIcon = Callback:GetQuestContextIcon(questInfo.questID)
			...
			function Callback:GetTint(questID)
				...
				if questID then
					...
					tint = { r = addon.CREF:GetSharedColor().RGB_PING_NEUTRAL.r, g = addon.CREF:GetSharedColor().RGB_PING_NEUTRAL.g, b = addon.CREF:GetSharedColor().RGB_PING_NEUTRAL.b, a = .5 }
				end
			...
			function Callback:GetCurrentState()
				...
				local isRangeProximity = (distance < 200)
		...
		do -- SET
			do -- GENERAL
				...
				function Callback:Waypoint_Reset()
			...
			do -- WAYPOINT
				...
				function Callback:Waypoint_SetContext(image, tintColor)
					Frame.REF_WAYPOINT_CONTEXT:SetInfo(image, tintColor)
				end
			...
			do -- PINPOINT
				...
				function Callback:Pinpoint_SetContext(image, tintColor)
					Frame.REF_PINPOINT_BACKGROUND_CONTEXT:SetInfo(image, tintColor)
				end
			end
		end
		...
		do -- LOGIC
			...
			function Callback:Update(superTrackingChanged)
				if superTrackingChanged then
				...
				local distance = C_Navigation.GetDistance()
				local distanceScaleModifier = Callback:GetDistanceScale(distance, 2000, .125, .125, 1.5, 1)
				...
				if state == "INVALID" or state == "INVALID_RANGE" then
					...
					if state == "QUEST_PROXIMITY" then
						if isNewState then
							...
							addon.C.Sound.Script:PlaySound(SOUNDKIT.UI_MAP_WAYPOINT_BUTTON_CLICK_ON)
						end
					elseif state == "PROXIMITY" then
						if isNewState then
							if not Frame_Pinpoint.hidden then
								Frame_Pinpoint:HideWithAnimation(id, true)
							end
					...
					elseif state == "QUEST_AREA" then
						if isNewState then
							...
							addon.C.Sound.Script:PlaySound(SOUNDKIT.UI_MAP_WAYPOINT_BUTTON_CLICK_OFF)
						end
					...
					if not Frame_Waypoint.hidden then
						...
						Callback:Waypoint_SetDistanceText(addon.C.API.Util:FormatNumber(string.format("%.0f", distance)) .. " yds")
						if questInfo then Callback:Waypoint_SetContext(questInfo.contextIcon.texture, tintColor) end
						if questInfo then Callback:Waypoint_SetContextVFX(questInfo.completed and "QuestCompletion" or "") end
						Callback:Waypoint_SetType(questInfo and "CONTEXT" or "WAYPOINT")
					end
					...
					if not Frame_Pinpoint.hidden then
						if questInfo then
							...
							if isComplete then
								if not currentQuestObjective then
									text = currentQuestObjective
								else
									text = L["Pinpoint - Quest - Complete"]
								end
							else
								if currentQuestObjective then
								...
								elseif isComplete then
									if currentWaypointObjective then
										...
										text = L["Waypoint - Quest - Complete"]
									end
								end
							end
							...
							Callback:Pinpoint_SetContext(questInfo.contextIcon.texture, tintColor)
						end
					end
				end

				Frame_Waypoint:SetScale(distanceScaleModifier)
			end
		end
	end
			...
			do -- SHOW
				...
				function Frame_Waypoint:ShowWithAnimation(id, skipAnimation)
					...
					if skipAnimation then
						...
						addon.C.Animation:Alpha( { ["frame"] = Frame.REF_WAYPOINT_CONTEXT, ["duration"] = .25, ["from"] = 0, ["to"] = 1, ["ease"] = "EaseExpo", ["stopEvent"] = function() return Frame_Waypoint:ShowWithAnimation_StopEvent(id) end } )
			...
			do -- HIDE
				...
				function Frame_Pinpoint:HideWithAnimation(id, skipAnimation)
					...
					if skipAnimation then
						...
						addon.C.Animation:Scale( { ["frame"] = Frame.REF_PINPOINT_BACKGROUND_CONTEXT, ["duration"] = .5, ["from"] = Frame.REF_PINPOINT_BACKGROUND_CONTEXT:GetScale(), ["to"] = 1.25, ["ease"] = "EaseExpo", ["stopEvent"] = function() return Frame_Pinpoint:HideWithAnimation_StopEvent(id) end } )
						addon.C.Animation:Alpha( { ["frame"] = Frame.REF_PINPOINT_BACKGROUND_CONTEXT, ["duration"] = .25, ["from"] = Frame.REF_PINPOINT_BACKGROUND_CONTEXT:GetAlpha(), ["to"] = 0, ["ease"] = "EaseExpo", ["stopEvent"] = function() return Frame_Pinpoint:HideWithAnimation_StopEvent(id) end } )
						addon.C.Animation:Alpha( { ["frame"] = Frame.REF_PINPOINT_BACKGROUND_ARROW, ["duration"] = .25, ["from"] = Frame.REF_PINPOINT_BACKGROUND_ARROW:GetAlpha(), ["to"] = 0, ["ease"] = "EaseExpo", ["stopEvent"] = function() return Frame_Pinpoint:HideWithAnimation_StopEvent(id) end } )
						...
						addon.C.Animation:Alpha( { ["frame"] = Frame.REF_PINPOINT_FOREGROUND, ["duration"] = .25, ["from"] = Frame.REF_PINPOINT_FOREGROUND:GetAlpha(), ["to"] = 0, ["ease"] = "EaseExpo", ["stopEvent"] = function() return Frame_Pinpoint:HideWithAnimation_StopEvent(id) end } )
					end
	...
	do
		CallbackRegistry:Add("WAYPOINT_CONFIG_SCALE", function()
			local variable = addon.C.Database.Variables.DB_GLOBAL.profile.WAYPOINT_SCALE
			Frame:SetScale(variable)
		end)
	end
